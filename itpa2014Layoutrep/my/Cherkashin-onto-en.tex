...
%    Alexander Zamashikov\IEEEauthorrefmark{3}
  ...
  \IEEEauthorblockA{\IEEEauthorrefmark{2}Irkutsk State University,
    Irkutsk, Gagarina blvd., 20, 664003, Russia}%
...
% Positive aspects of the paper: The paper have shown some results obtained using and description language for building an Information Systems based on an existing database scheme and additional semantic description. Modules for database editing, query construction and report generation are described.
% Objections to the paper: There are several useless references (which I think) that should be removed, i.e. reference to Django in Introduction and references to Wikipedia. Conclusion is not an appropriate section for references. The figures are quite small and unreadable but the more important problem is that they are positioned too far from the text that reference them.

% The paper is somehow confusing. Some sentences claim that database for IS already exists and some that it should be produced in one of the steps.

% There are several typos in text (bust => must, ajusts => adjusts, munidimentional => multidimensional, Microdoft => Microsoft, sown => ?).,  especially in Section III.

% Also in Section III the second and the third paragraph are useless (and references in it) because it is described better in later paragraphs.

% In Section III.B it is not clear if Microsoft Access is used as an comparision for query builder or it has been used?
% In III.C there are several sentences obviously incorrect as
% “The data converts into a tree of indicators.“ Someone converts data or data converts something?
% “Each node of the tree corresponds data selected in a projection”. corresponds to?
% Section IV is called A usage example but I cannot see how techniques from previous sections are applied. I suggest removing this section of improving it.
% References are very weak. After removing useless references (1, 2, 5, 6, 8?), the paper would have only 6-7 references with 3 of them in Russian.
% Technical Quality: Adequate
% Originality: Adequate
...
\begin{abstract}
Software development technologies evolved from concrete algorithm design and their programmatic implementation to modeling the whole information systems (IS).  The modeling approaches allows developers to describe the system in terms of a domain, software design, properties of implementation platforms, and standard ways of the IS development such as programming traditions in a concrete firm.  In the case if a new IS is being constructed, having an already implemented database, Data Driven Engineering (DDE) is used.  The model for IS is constructed by importing database structure and adding a semantic layer description reflecting the database structure to the domain terms.

We describe a DDE-approach based on declarative specifications of existing database schema and properties of the target application, as well as interpretation of the description into IS components.  The interpretation in various aspects allows developer to construct user interface for database modification, subsystem of SQL query construction in terms of domain, etc.  An example of the technique application is presented.
...
\bibitem{django} The Web framework for perfectionists with deadlines | Django.   URL:~\url{https://www.djangoproject.com/} (access date: 10.02.2015)
\bibitem{dde} M.Brambilla, J.Cabot, M.Wimmer, ``Model-Driven Software Engineering in Practice.'' Morgan \& Claypool Publ. Co., 2012, 182~p.
\bibitem{aga1} V.N.Agafonov. ``Program specification: concepts and their organization.'' Novosibirsk ``Nauka'' publ. co. 1987. 240~p. (in Russian)
\bibitem{aga2} V.N.Agafonov. ``Requirements and specifications in program development.'' Moscow ``Mir'' publ. co. 1984. (in Russian)
\bibitem{qt} Qt Project. URL:~\url{http://qt-project.org/}. (access date: 02.02.2015)
\bibitem{gtk} The GTK+ Project. URL:~\url{http://www.gtk.org/}. (access date: 02.02.2015)
...
\bibitem{b42}GIS Panorama. URL:~\url{http://www.gisinfo.net/}. (access date: 21.02.2015)
